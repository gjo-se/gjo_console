...
./scripts/helper/echoHeader.sh "Create rewritemaps" "./scripts/createRewriteMaps.sh [-t|--transfer]"
...
echo "Creating rewritemaps..."
cd web > /dev/null
sudo -E -u www-data -g www-data php typo3/cli_dispatch.phpsh kaercher_seomanager -a createrewritemaps > /dev/null
cd - > /dev/null
cecho "[  OK  ]" $green
echo ""
...
if [ $config_transfer == 1 ]; then
    ...
    aws s3 sync rewritemaps/ s3://redesign-cms/sync/rewritemaps/
    cecho "[  OK  ]" $green
fi;
